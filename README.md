...
### Build19 remaining reproducibility and recovery property-context work

The required recovery metadata-policy carveout currently lives as an uncommitted change in the detached AOSP `system/sepolicy` checkout at `android-14.0.0_r75`. That checkout has only the AOSP remote, so the Build19 policy change is not yet reproducible from the LieppOS device tree.

TrustKernel property contexts are correctly generated by Soong and installed into `vendor/etc/selinux/vendor_property_contexts`; those two artifacts are byte-identical. However, `recovery/root/vendor_property_contexts` remains the older 826-byte artifact and contains none of the TrustKernel property labels, even after rebuilding `sepolicy.recovery`.

Build19 therefore still needs a reproducible mechanism for the platform SELinux patch and a verified recovery-ramdisk property-context packaging path before final packaging.

### Build19 SELinux patch reproducibility and property contexts

The recovery metadata SELinux carveout is now stored as a device-tree patch. Starting from a clean AOSP `system/sepolicy` checkout, `build/envsetup.sh` sources the device `vendorsetup.sh`, which applies the patch automatically. Re-sourcing the environment is idempotent and does not reapply or fail.

From that automatically patched clean state, `sepolicy.recovery` compiles successfully and generated recovery CIL still contains exactly `recovery -> vold_metadata_file:dir search` and `recovery -> vold_metadata_file:file { read getattr open }`.
